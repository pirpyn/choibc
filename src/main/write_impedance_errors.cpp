/* subroutine write_impedance_error(data,data_extended,hoibc_list)
    use iso_fortran_env, only: error_unit, output_unit
    use mod_hoibc_constants, only: wp, pi, speed_of_light, vacuum_impedance
    use mod_hoibc_data,only: data_type, HOIBC_CHAR_LEN, HOIBC_FILE_LEN, disp_data, check_data
    use mod_hoibc_math, only: norm, bubble_sort, linspace
    use mod_hoibc_write_csv, only: dump_to_csv, set_backend, set_arg
    use mod_hoibc_math_plane,     only: reflexion_infinite_plane,     impedance_from_reflexion_plane    => impedance_from_reflexion
    use mod_hoibc_math_cylinder,  only: reflexion_infinite_cylinder,  impedance_from_reflexion_cylinder => impedance_from_reflexion
    use mod_hoibc_math_sphere,    only: reflexion_infinite_sphere,    impedance_from_reflexion_sphere   => impedance_from_reflexion
    use mod_hoibc_math_plane,     only: impedance_infinite_plane,     reflexion_from_impedance_plane    => reflexion_from_impedance
    use mod_hoibc_math_cylinder,  only: impedance_infinite_cylinder,  reflexion_from_impedance_cylinder => reflexion_from_impedance
    use mod_hoibc_math_sphere,    only: impedance_infinite_sphere,    reflexion_from_impedance_sphere   => reflexion_from_impedance
    use mod_hoibc_class, only: hoibc_generic_list, hoibc_class, assignment(=)

    type(data_type),intent(in) :: data
    type(data_extended_type),intent(in) :: data_extended

    type(hoibc_generic_list),dimension(:),allocatable,intent(inout) :: hoibc_list

    class(hoibc_class),allocatable :: ibc
    integer :: nhoibc
    integer :: i
    integer, parameter :: line_width = 82

    real(wp),dimension(:),allocatable :: f1 !# The 1st fourier variable : kx, n, m
    real(wp),dimension(:),allocatable :: f2 !# The 2nd fourier variable : ky, kz, n
    real(wp),dimension(:),allocatable :: s1 !# f1/k0
    real(wp),dimension(:),allocatable :: s2 !# f2/k0

    integer :: i1
    integer :: i2
    real(wp) :: k0
    complex(wp),dimension(:,:,:,:),allocatable :: Z_ex, Z_ap
    complex(wp),dimension(:,:,:,:),allocatable :: R_ex, R_ap
    ...
    character(len=:),allocatable :: fmt_error_header
    character(len=:),allocatable :: fmt_error_value

    call disp_data(data)

    write(output_unit,*)
    write(output_unit,'(a)') '=================================================' 
    write(output_unit,'(a)') 'HOIBC: All IBC coefficients computed with success.'
    write(output_unit,'(a)') '================================================='
    write(output_unit,*)
    ...
    do i=1,nhoibc
      ...
      call ibc%set_fourier_variables(data,f1,f2,s1,s2)
      ! Display the coefficient to the standard output (stdout)
      call ibc%print_coeff()
      ! Display the SUC to stdout
      call ibc%disp_suc(data%optim%acc)
