...
namespace {
  ...
  void report_iteration(const alglib::real_1d_array &x, double func, void *ptr, bool reset){
    ...
    if (iter == 0){
      ...
      std::cout << string_format("%10s, ", "|dx|") << std::endl;;
    }
    ...
    std::cout << string_format("%10.3E, ", norm ) << std::endl;
...
namespace hoibc {
  ...
  void hoibc_class::print_suc(const real& tol, std::ostream& out){
    ...
    if (cle.size()){
      if (any(cle, x <= tol)){
        out << get_cmt() << string_format("    [OK] SUC, Negative inequality constraints, IN <= %10E",tol) << std::endl;
        for (std::size_t i = 0; i < cle.size(); i++){
          if (cle[i] <= tol){
            out << get_cmt() << string_format("    IN(%d) = %13.6E ! ",i+1,cle[i]) << sle[i] << std::endl;
          }
        }
      }
      if (any(cle, x > tol)){
        out << get_cmt() << string_format("  [FAIL] SUC, Positive inequality constraints, IN > %10E",tol) << std::endl;
        for (std::size_t i = 0; i < cle.size(); i++){
          if (cle[i] > tol){
            out << get_cmt() << string_format("    IN(%d) = %13.6E ! ",i+1,cle[i]) << sle[i] << std::endl;
          }
    ...
    if (ceq.size()){
      if (any(ceq, std::abs(x) <= tol)){
        out << get_cmt() << string_format("    [OK] SUC, Zero equality constraints, |EQ| <= %10E",tol) << std::endl;
        for (std::size_t i = 0; i < ceq.size(); i++){
          if (ceq[i] <= tol){
            out << get_cmt() << string_format("    EQ(%d) = %13.6E ! ",i+1,ceq[i]) << seq[i] << std::endl;

          }
        }
      }
      if (any(ceq, std::abs(x) > tol)){
        out << get_cmt() << string_format("  [FAIL] SUC, Non-zero equality constraints, |EQ| >  %10E",tol) << std::endl;
    ...
    if (cne.size()){
      if (any(cne, std::abs(x) <= tol)){
        out << get_cmt() << string_format("    [OK] SUC, Non-zero equality constraints, |NE| => %10E",tol) << std::endl;
        for (std::size_t i = 0; i < cne.size(); i++){
          if (cne[i] <= tol){
            out << get_cmt() << string_format("    NE(%d) = %13.6E ! ",i+1,cne[i]) << sne[i] << std::endl;
          }
        }
      }
      if (any(cne, std::abs(x) > tol)){
        out << get_cmt() << string_format("  [FAIL] [FAIL] SUC, Zero equality constraints, |NE| < %10E",tol) << std::endl;
        for (std::size_t i = 0; i < cne.size(); i++){
          if (cne[i] > tol){
            out << get_cmt() << string_format("    NE(%d) = %13.6E ! ",i+1,cne[i]) << sne[i] << std::endl;
          }
  ...
  void get_matrices_I(const std::size_t& n1, const std::size_t& n2, big_matrix<real>& I, big_matrix<real>& I1, big_matrix<real>& I2){
    I.resize(0);
    I1.resize(0);
    I2.resize(0);
    matrix<real> sI;
