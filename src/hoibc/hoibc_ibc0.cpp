...
namespace hoibc {
  ...
  void hoibc_ibc0::get_coeff_no_suc(const array<real>& f1, const array<real>& f2, const big_matrix<complex>& gex, const real& k0){
  ...
  big_matrix<complex> hoibc_ibc0::get_impedance(const real& k0, const array<real>& f1, const array<real>& f2){
    big_matrix<real> I;
  ...
  void hoibc_ibc0::array_to_coeff(const alglib::real_1d_array& x){
    this->coeff.a0 = complex(x[0],x[1]);
  }

  void hoibc_ibc0::coeff_to_array(alglib::real_1d_array& x){
    x.setlength(2);
    x[0] = std::real(this->coeff.a0);
    x[1] = std::imag(this->coeff.a0);
  }

  void hoibc_ibc0::get_suc(array<real>& cle, array<real>& ceq, array<real>& cne, array<std::string>& sle, array<std::string>& seq, array<std::string>& sne){
    cle.resize(1);
    sle.resize(1);
    ceq.resize(0);
    seq.resize(0);
  ...
  void hoibc_ibc0::disp_coeff(std::ostream& out){
    out << " Z = a0*I" << std::endl;
